...
export class UserService {
  ...
  async getUserById(userId: number, findId: number): Promise<any> {
    ...
    if (currUser.id === findUser.id) return this.getUserDataById(userId);
  ...
  async getStatsById(id: number) {
    ...
    const games = user.Games.map(async (game) => {
  ...
  async removePrivateChannel(userId1: number, userId2: number) {
    ...
    const channelId = `__private__@${Math.min(userId1, userId2)}+${Math.max(
      userId1,
      userId2,
    )}`;
    ...
    const updatedPrivateChannels1 = user1.privateChannels.filter(
      (c) => c !== channelId,
    );
    const updatedPrivateChannels2 = user2.privateChannels.filter(
      (c) => c !== channelId,
    );
  ...
  async blockFriend(id: number, friendID: number) {
    ...
    if (!isExist)
      throw new NotFoundException('User is not your friend or already blocked');
  ...
  async unblockFriend(id: number, friendID: number) {
    ...
    await this.prisma.user.update({
      ...
      data: {
        Blocked: {
          set: user.Blocked.filter((blockedId) => blockedId !== friend.id),
        },
      },
    });
    ...
    const privateChannelId = `__private__@${Math.min(
      user.id,
      friend.id,
