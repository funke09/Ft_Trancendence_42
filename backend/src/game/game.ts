...
export class Game {
  ...
  updatePaddles() {
    if (!this.client1 || !this.client2) this.stopGame(false);

    if (
      this.playerDir.player1.up &&
      this.player1.position.y - PaddleHieght / 2 > 0
    ) {
      ...
      this.playerDir.player1.down &&
      this.player1.position.y + PaddleHieght / 2 < Height
    ) {
      Body.setPosition(this.player1, {
        x: this.player1.position.x,
        y: this.player1.position.y + PaddleSpeed,
    ...
    if (
      this.playerDir.player2.up &&
      this.player2.position.y - PaddleHieght / 2 > 0
    ) {
      ...
      this.playerDir.player2.down &&
      this.player2.position.y + PaddleHieght / 2 < Height
    ) {
      Body.setPosition(this.player2, {
        x: this.player2.position.x,
        y: this.player2.position.y + PaddleSpeed,
  ...
  updateBall = () => {
    if (this.ballDir.y === 0 && this.ballDir.x !== 0) this.ballDir.y = 1;
    else if (this.ballDir.x === 0 && this.ballDir.y !== 0) this.ballDir.x = 1;
  ...
  onCollisionStart = (event) => {
    ...
    pairs.forEach((pair: any) => {
      ...
      if (pair.bodyA.id == 5 && pair.bodyB.id == 6) {
        collide =
          (pair.bodyA.position.y - pair.bodyB.position.y) / (PaddleHieght / 2);
        const angle = (Math.PI / 3) * collide;
  ...
  onCollisionEnd = (event) => {
    const resetBall = () => {
      ...
      this.countDown();
      setTimeout(() => {
        this.ballDir = ballDir[diffScore];
      }, 5000);
    };
    ...
    if (p1 >= 5 || p2 >= 5) {
      this.client1 &&
        this.client1.emit('gameMsg', `${p1 > p2 ? 'WINNER' : 'GAME OVER'}`);
      this.client2 &&
        this.client2.emit('gameMsg', `${p2 > p1 ? 'WINNER' : 'GAME OVER'}`);
  ...
  reconnectPlayer(username: string, client: any): void {
    if (username === this.p1Username) this.client1 = client;
    else if (username === this.p2Username) this.client2 = client;
