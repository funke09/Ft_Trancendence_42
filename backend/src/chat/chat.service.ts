...
export class ChatService {
	...
	async jwtDecoe(client: Socket, context?: ExecutionContext) : Promise<any> {
		...
		try {
			...
			if (context)
				context.switchToWs().getData().userId = decoded.uid;
	...
	async createChannel(username: string, to: number, channelId: string) : Promise<string> {
	...
	async addFriend(id: number, friendRequest: string, server: Server) {
        ...
        let notification_SIDE = await this.prisma.notifs.create({
            data: {
                type: 'friendRequest',
	...
	async emitToUser(server: Server, username: string, event: string, data: SocketResDto | NotifsDto) : Promise<void> {
		const user = await this.prisma.user.findUnique({ where: {username: username}});
		if (!user) [];
		if (user.socketId)
